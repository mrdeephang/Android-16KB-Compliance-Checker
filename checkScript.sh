...
# App Bundle (AAB) Processing
if [[ "${dir}" == *.aab ]]; then
  trap 'cleanup_trap' EXIT

  print_section "$PACKAGE" "App Bundle Analysis" "Processing Android App Bundle file"
  ...
  print_status "processing" "Extracting native libraries from AAB..."
  if ! unzip -q "${dir}" "base/lib/*" -d "${tmp}" 2>/dev/null; then
    # Some bundles might not have 'base/' prefix if they are processed differently
    if ! unzip -q "${dir}" "lib/*" -d "${tmp}" 2>/dev/null; then
      print_summary_box "NO NATIVE LIBRARIES FOUND" "success" \
        "${PARTY} Your app bundle contains only Java/Kotlin code!" \
        "" \
        "${CHECK_MARK} Apps without native libraries automatically support 16KB devices" \
        "${CHECK_MARK} No additional changes required for Google Play compliance"
      cleanup_trap 0
